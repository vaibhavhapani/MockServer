...
function checkAuthHeader(req, res, next) {
  ...
  // For now, we won't validate the token but just move forward
  if (token) {
    next();
  } else {
...
app.get("/userInfo", checkAuthHeader, (req, res) => {
  // Set response header for JWT token
  res.set("Content-Type", "application/jwt");
  res.set("X-TraceId", "102210010936033279"); // Hardcoded TraceId for now

  // Send the hardcoded JWT as the response body
  res.send(jwtToken);
});
